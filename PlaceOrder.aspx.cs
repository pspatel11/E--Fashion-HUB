...
public partial class PlaceOrder : System.Web.UI.Page
{
    ...
    protected void btnPlaceOrder_Click(object sender, ImageClickEventArgs e)
    {
        ...
        {
            ConstantVariable cd = new ConstantVariable();
            string transType = "offline"; //online

            string responseURL = "";
            if (transType == "offline")
            {
                int regid = Convert.ToInt32(Session["UserId"].ToString());
                int itemId = Convert.ToInt32(hdnProductId.Value);
                decimal price = Convert.ToDecimal(lblFinalPrice.Text.ToString());
                int quantity = Convert.ToInt32(txtQuntity.Text.ToString());

                string queryParam = "?transType="+ transType + "&regid=" + regid + "&itemId=" + itemId + "&price=" + price + "&quantity=" + quantity;

                string url = ConfigurationManager.AppSettings["SuccessURL"].ToString();
                responseURL = url + queryParam;
            }
            else
            {
                responseURL = cd.PayWithPayPal(transType, lblFinalPrice.Text.ToString(), lblProductName.Text, Session["Name"].ToString(), "", "", "INR", txtQuntity.Text.ToString(), Convert.ToInt32(Session["UserId"].ToString()), Convert.ToInt32(hdnProductId.Value));
                //string responseURL = ConstantVariable.GetItemNameAndCost(lblProductName.Text, lblFinalPrice.Text, "Nusery", "Ahmedabad", "Gujarat");
            }
