...
public partial class plant : System.Web.UI.Page
{
    ...
    public void getData()
    {
        ...
        if (Session["HolderId"] != null)
        {
            ...
            string s = @"select plant_id,plant_name,p.Category_Id,c.category_name as category_name,Status_check,price,image1,image2 
                        from Item_Details p
                        inner join Category c on p.Category_Id = c.Category_Id and c.category_type = 0 ";
        ...
        try
        {
            if (ddlCategory.SelectedValue != null)
            {
                ...
                if (Image2FileUpload.HasFile)
                {
                    ...
                    string fileExtension = Path.GetExtension(fileName);
                    newFileName2 = txtPlantName.Text.Trim() + "_2_" + Guid.NewGuid() + fileExtension;
                ...
                cmd.Parameters.AddWithValue("@plantation_info", txtPlantInfo.Text.Trim());
    ...
    protected void btnUpdate_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            cmd.CommandType = CommandType.StoredProcedure;
            cmd.Parameters.AddWithValue("@plant_Id", hdnPlantId.Value.Trim());
            cmd.Parameters.AddWithValue("@Category_Id", (ddlCategory.SelectedValue == "") ? null : ddlCategory.SelectedValue);
            cmd.Parameters.AddWithValue("@plant_name", txtPlantName.Text.Trim());
            cmd.Parameters.AddWithValue("@Description", txtDescription.Text.Trim());
            cmd.Parameters.AddWithValue("@care_info", txtCareInfo.Text.Trim());
    ...
    protected void btnDelete_Click(object sender, EventArgs e)
    {
        try
        {
            SqlConnection cn = new SqlConnection(ConfigurationManager.ConnectionStrings["NurseryConnection"].ConnectionString);
            cn.Open();
            string query = "delete from Item_Details where plant_Id = " + hdnPlantId.Value;
    ...
    protected void grdPlant_SelectedIndexChanged(object sender, EventArgs e)
    {
        ...
        hdnPlantId.Value = grdPlant.SelectedDataKey.Value.ToString();

        SqlConnection cn = new SqlConnection(ConfigurationManager.ConnectionStrings["NurseryConnection"].ConnectionString);
        cn.Open();
        string s = @"select plant_Id,plant_name ,p.Category_Id ,Description,care_info,plantation_info, Status_check,price from Item_Details p where plant_Id = " + hdnPlantId.Value;
        ...
        if (dt != null && dt.Rows.Count > 0)
        {
            ...
            txtPlantInfo.Text = dt.Rows[0]["plantation_info"].ToString();
